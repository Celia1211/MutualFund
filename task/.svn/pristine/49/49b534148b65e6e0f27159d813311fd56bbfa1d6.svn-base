...
public class TransactionDao extends GenericDao<Transaction> {
    ...
    public List<Transaction> findPendingCashTrans(Date orderDate) {
        return em.createQuery(
                "SELECT e FROM Transaction e WHERE  e.executeDate = null AND e.transactionType in ('REQUEST_CHECK', 'DEPOSIT') AND e.status = 'PENDING' AND e.orderDate < :orderDate ",
                genericType).setParameter("orderDate", orderDate).getResultList();
    }

    public List<Transaction> findPendingFundTrans(Long fundId, Date orderDate) {
        return em
                .createQuery(
                        "SELECT e FROM Transaction e WHERE  e.executeDate = null AND e.fund.fundId = :fundId AND e.transactionType in ('SELL', 'BUY') AND e.status = 'PENDING' AND e.orderDate < :orderDate",
                        genericType)
                .setParameter("fundId", fundId).setParameter("orderDate", orderDate).getResultList();
    }
}

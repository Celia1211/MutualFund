...
public final class Utils {
    ...
    public static void checkRequiredField(String value, String fieldName, List<String> errorMsgs) {
        ...
        if (value.length() > 100) {
            errorMsgs.add("The length of " + fieldName + " cannot greater than 100");
            return;
        }
    ...
    public static void checkNewPassword(String password, String confirmPassword, List<String> errorMsgs) {
        ...
        if(!password.matches("[a-zA-Z0-9]+")){
            errorMsgs.add("Password should contain alphanumeric characters only!");
            return;
        }
    ...
    public static void checkDateFormta(String date, List<String> errorMsgs) {
        try {
            df.parse(date);
    ...
    public static void checkDoubleInputStr(String[] vals, List<String> errorMsgs) {
        for (String val : vals) {
            ...
            try {
                ...
                errorMsgs.add("Some of price value format are incorrect or empty!");
                return;
            }

            if (doubleVal <= 0) {
                errorMsgs.add("All prices should be greater than 0");
                return;
            }
            
            if (doubleVal >10000){
                errorMsgs.add("All prices should be less than 10,000");
                return;
            }
        }
    }
